...
class AccountActivatedMail extends Mailable
{
        ...
        $url =
            optional(
                Post::isType(config('cmstypes.page'))
                    ->isKey(config('cmstemplates.account_activated'))
                    ->first()
    ...
    public function build()
    {
        return $this->from(config('mail.from.address'), app_name())
            ->subject(__('cms::emails/account-activated.subject', ['name' => app_name()], $this->lang_code))
            ->view('cms::emails.account-activated.html')
            ->text('cms::emails.account-activated.plain');
    }
}
